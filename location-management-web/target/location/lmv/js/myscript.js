...
$(document).ready(function(){
	...
	// resetting modal data input fields when hiding
	//TODO do it only for data entry modals to save performance
	  $(".modal").on("hidden.bs.modal", function() {
		    console.log("Resetting fields for popup");
		    var inputs = $('#'+this.id+' input, textarea, select').not(':input[type=hidden], :input[type=button], :input[type=submit], :input[type=reset]');
	...
	$("#addAlllocationTypes").bind("click", function() {
		...
		if(document.getElementById("addlocationTypeName").value !== "" ) {
			if(document.getElementById("addlocationTypeName").value.length >= 3 && document.getElementById("addlocationTypeName").value.length <= 50 && pattern2.test(document.getElementById("addlocationTypeName").value) && pattern.test(document.getElementById("addlocationTypeLevel").value )) {
				var dataNode = { isEditable: true, name: document.getElementById("addlocationTypeName").value, level: document.getElementById("addlocationTypeLevel").value, description: document.getElementById("addlocationTypeDescription").value };
				$.ajax({
					url: "rest/newlocationtype",
					type: "POST",
					data: dataNode,
					success : function(result) {
						var data = JSON.parse(result);
						...
						var level = dataNode.level;
						var description = dataNode.description;
						var node = {isEditable: true, id: id, name: name, level: level, description: description };
	...
	$("#addAlllocationAttributeTypes").bind("click", function() {
		...
		if(document.getElementById("addlocationAttributeTypeName").value !== "") {
			if(( document.getElementById("addlocationAttributeTypeName").value.length >= 3 && document.getElementById("addlocationAttributeTypeName").value.length <= 100 && pattern.test(document.getElementById("addlocationAttributeTypeName").value)) && pattern.test(document.getElementById("addlocationAttributeTypeDisplayName").value) ) {
				...
				$.ajax({
					url: "rest/newlocationattributetype",
					type: "POST",
					data : dataNode,
					success : function(result) {
						var data = JSON.parse(result);
	...
	$("#updateAlllocationAttributeTypes").bind("click", function() {
		if(document.getElementById("updatelocationAttributeTypeName") != null) {
			if(document.getElementById("updatelocationAttributeTypeName").value !== "" ) {
				...
				$.ajax({
					url: "rest/updatelocationattributetype",
					type: "POST",
...
// isEditable param in function would override the location type setting
function addHoverDom(treeId, treeNode, isEditable) {
	if(!isEditable && !treeNode.isEditable){
	...
	if(treeId === "treeDemo") {
		...
		try{//TODO doesnot work when location is just added and somehow edit button is missing
...
function createTreeToolButton(itemId,targetId,title,imageName,clickFunction){
    return $("<button class='toolbar-action-button' style='background-image: url(resources/images/"+imageName+".png)' " +
...
function addAll(id) {
    ...
    if(id != "" && name != "" && type != "") {
    	var treeObj = $.fn.zTree.getZTreeObj("treeDemo");
 	    ...
 	    // TODO add all properties those are associated with other nodes
    	node = {tId:id, id: id, name: name, shortName: shortName, fullName: fullName, 
	    ...
	    addHoverDom("treeDemo", newNode[0], true);
    }
}
function GenerateTable(myNodes, parentName, parentId) {
	...
	if(parentName === "VOIDED LOCATIONS") { 
		...
		if(myNodes){
			while(myNodes[i] !== undefined) {
		    	...
		    	editArray.push("<a name='editBtn' id='editBtn' class='icon' style='cursor:pointer;'><img alt='edit' src='resources/images/edit-icon.png' class='icon' width='15px' height='15px'></a>");
				}
		    	idArray.push(myNodes[i].id);
		        nameArray.push(myNodes[i].name);
		        shortNameArray.push(myNodes[i].shortName);
    ...
    for (var i = 0; i < rowsAfterHeader; i++) {
        ...
        if(parentName === "VOIDED LOCATIONS") { }
            ...
            cell.innerHTML = "<a name='editBtn' id='editBtn' class='icon' title='Update "+nameArray[i]+"' onclick='editTableRow(\""+id+"\");' style='cursor:pointer;'><img alt='edit' src='resources/images/edit-icon.png' class='icon' width='15px' height='15px'></a>";
...
function addToTable(node) {
	if(document.getElementById("datatable") == null) {}
		...
		var edit = "<a name='editBtn' id='editBtn' class='icon' title='Update "+node.name+"' onclick='editTableRow(\""+id+"\");' style='cursor:pointer;'><img alt='edit' src='resources/images/edit-icon.png' class='icon' width='15px' height='15px'></a>";
...
function editTableRow(cellId) {
    ...
    document.getElementById(cellId).innerHTML = "<a name='saveBtn' id='saveBtn' title='Save' onclick='saveTableRow();' class='icon' style='cursor:pointer;'><img alt='edit' src='resources/images/tick.png' class='icon' width='15px' height='15px' ></a>";
...
function saveTableRow() {
    ...
    if (r === true) {
		...
		if(errorStr !== "") { alertDIV("warning", "ERROR: ", errorStr); }
			...
			document.getElementById(globalRowId).cells[0].innerHTML = "<a name='editBtn' id='editBtn' class='icon'  title='Update "+name+"' onclick='editTableRow(\""+globalCellId+"\");' style='cursor:pointer;'><img alt='edit' src='resources/images/edit-icon.png' class='icon' width='15px' height='15px'></a>";
			for (var i = 0; i < headerArray.length; i++) {
...
function updateName(id) {
	if(id==="updatelocationtype") { 
    	...
    	nameHtml += "<a name='saveNameBtn' id='saveNameBtn' class='icon' title='Save Name'style='cursor:pointer;' onclick='saveUpdatedName(\""+id+"\");'><img alt='edit' src='resources/images/tick.png' class='icon' style='width: 15px; '></a>";
    	nameHtml += "<a name='cancelNameBtn' id='cancelNameBtn' class='icon' title='Cancel Name' style='cursor:pointer;' onclick='cancelUpdatedName(\""+id+"\");'><img alt='edit' src='resources/images/undo.png' class='icon' style='width: 15px; '></a>";
    	nameHtml += "</div>";
		...
		nameHtml += "<div style='float: right; position: relative; top: -30px;'>";
    	nameHtml += "<a name='saveNameBtn' id='saveNameBtn' title='Save Name' class='icon' style='cursor:pointer;' onclick='saveUpdatedName(\""+id+"\");'><img alt='edit' src='resources/images/tick.png' class='icon' style='width: 15px; '></a>";
    	nameHtml += "<a name='cancelNameBtn' id='cancelNameBtn' title='Cancel Name' class='icon' style='cursor:pointer;' onclick='cancelUpdatedName(\""+id+"\");'><img alt='edit' src='resources/images/undo.png' class='icon' style='width: 15px; '></a>";
    	nameHtml += "</div>";
...
function cancelUpdatedName(id) {
	if(id==="updatelocationtype") { 
		...
		nameHtml += "<a name='renameBtn' id='renameBtn' class='icon' title='Edit Name' style='cursor:pointer;' onclick='updateName(\""+id+"\");'><img alt='edit' src='resources/images/edit-icon.png' class='icon' style='width: 30px; float: right; position: relative; top: -30px;'></a>";
		document.getElementById("updatelocationTypeDiv").innerHTML = nameHtml;
		setFocus(id);
	}
	else if(id==="updatelocationattributetype") { 
...
function saveUpdatedName(id) {
	if(id==="updatelocationtype") { 
		...
		if(pattern.test(saveName)) {
			$.ajax({
				url: "rest/updatelocationtypename",
				type: "POST",
		...
		if(pattern.test(saveName)) {
			$.ajax({
				url: "rest/updatelocationattributetypename",
				type: "POST",
				data : { id: saveId, name: saveName },
				success : function(result) {
					var data = JSON.parse(result);
